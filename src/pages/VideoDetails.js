...
import { fetchDataFromApi, fetchVidFromApi } from "../utils/api";
...
const VideoDetails = () => {
  ...
  const fetchVideoDetails = async () => {
    setLoading(true);
    const data = await fetchVidFromApi(`video/info?id=${id}`);
    // console.log(data);
    setVideo(data);
    setLoading(false);
    console.log('vid_datta', video)
  };

  // fetch Data for the suggestion videos
  const fetchRelatedVideos = async () => {
    setLoading(true);
    const data = await fetchVidFromApi(`related?id=${id}`);
    // console.log(data);

    setRelatedVideos(data.data);

    setVideo(data.meta);
    console.log("data", relatedVideos, video)
    setLoading(false);
  };

  return (
    <div className="flex flex-row justify-center h-fit bg-white dark:bg-black">
      <div className="w-full max-w-[1280px] flex flex-col lg:flex-row ">
        <div className="flex flex-col lg:w-[calc(100%-350px)] px-4 py-3 lg:py-6 overflow-y-auto">
          <div className="h-[200px] md:h-[400px] lg:h-[400px] ml-[-16px] lg:ml-0 mr-[-16px] lg:mr-0">
            <ReactPlayer
